...
export function makeProvider(env,fetcher=fetch){
 ...
 async function classifyJev(email){
  ...
  if(!CATEGORIES.includes(category)||typeof confidence!=='number'||confidence<0||confidence>1||typeof intent!=='number'||intent<0||intent>1){const e=Error('Jev response did not satisfy the classification contract.');e.code='AI_INVALID_OUTPUT';throw e}
  ...
  const needsReview=!complete||confidence<0.8||category==='BL_COMPARISON'&&(!gate||intent<0.8)||!quote;
  return {category,quote,reason:'Jev routing; confidence '+confidence.toFixed(2)+'; comparison intent '+intent.toFixed(2)+'.',needsReview,provider:'jev',gate,keyword_gate:gate?'hit':'no_hit',body_coverage:{characters:email.subject.length+email.body.length,complete},promptVersion:'jev-route-1'};
 }
