...
export function makeProvider(env,fetcher=fetch){
 ...
 async function call(schema,instructions,input,image,repair=false){
  ...
  const send=body=>fetcher(base.href.replace(/\/$/,'')+'/chat/completions',{method:'POST',headers:{Authorization:'Bearer '+api.key,'Content-Type':'application/json'},body:JSON.stringify(body),signal:AbortSignal.timeout(45000),redirect:'manual'});
