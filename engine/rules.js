...
import {parseNumber,invariantNumericComparison,groupedIntegerAgreement} from './numbers.js';
export const KEYS=['shipper','consignee','notify_party','port_of_loading','port_of_discharge','container_count','gross_weight_kg'];
export const RULE_VERSION='verity-rules-1.10';
...
export function compareDocuments(si,bl,profiles={si:'unset',bl:'unset'}){
 return Object.fromEntries(KEYS.map(key=>{
  ...
  if(key==='gross_weight_kg'&&av.ok&&bv.ok){
   ...
   if(!numericDecision&&unitSI?.unit===unitBL?.unit&&groupedIntegerAgreement(a?.raw,b?.raw))numericDecision='MATCH';
  ...
  if(key==='gross_weight_kg')qa.push({rule:'numeric_interpretations',status:numericDecision?'pass':'flag',decision:numericDecision,si:x.candidates||[],bl:y.candidates||[],sourceSI:numericSI,sourceBL:numericBL,comparisonPolicy:'exact_decimal_no_rounding_tolerance; identical source tokens match only when all converted candidates are identical; same-unit plain integer equals canonical comma-grouped integer'});
