...
const PORTS={SGSIN:['SINGAPORE'],MYPKG:['PORT KLANG','PORT KELANG'],USLAX:['LOS ANGELES'],USPDX:['PORTLAND OREGON','PORTLAND OR'],AUPTJ:['PORTLAND VICTORIA','PORTLAND VIC'],CNSHA:['SHANGHAI'],CNNGB:['NINGBO'],HKHKG:['HONG KONG'],NLRTM:['ROTTERDAM']};
...
export function normalizeField(key,raw,profile='unset'){
 ...
 if(key.startsWith('port_')){
  ...
  const without=codes.reduce((t,c)=>t.replace(c,''),text).replace(/[()\[\],]/g,'').trim();
  if(text==='PORTLAND')return fail('AMBIGUOUS','alias_collision');
  const named=Object.entries(PORTS).filter(([,aliases])=>aliases.some(a=>name(a)===without)).map(([c])=>c);
  if(codes.length){if(codes.length!==1||valid.length!==1)return fail('AMBIGUOUS','alias_not_found');if(without&&(!named.length||named[0]!==valid[0]))return fail('AMBIGUOUS','port_name_code_conflict');return good(valid[0])}
...
export function validateEvidence(field,doc,key){
 ...
 const labels={shipper:/\b(shipper|exporter)\b/i,consignee:/\bconsignee\b/i,notify_party:/\bnotify(?: party)?\b/i,port_of_loading:/\b(port of loading|loading port|load port|pol)\b/i,port_of_discharge:/\b(port of discharge|discharge port|pod)\b/i,container_count:/\b(container count|no\.? of containers|number of containers|containers)\b/i,gross_weight_kg:/\bgross (weight|mass)\b/i};
 ...
 if(field.entity&&Object.values(field.entity).some(v=>v&&!norm(field.raw).includes(norm(v))))return fail('UNREADABLE','evidence_not_located');
...
export function compareDocuments(si,bl,profiles={si:'unset',bl:'unset'}){
 return Object.fromEntries(KEYS.map(key=>{
  ...
  const entityRaw=f=>f?.entity?f.entity.name+' '+f.entity.qualifier:f?.raw;
