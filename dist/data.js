...
function preset(id,company,type,changes={},extra={}){const fields=makeFields();for(const [key,val] of Object.entries(changes))Object.assign(fields[key],val);const record={id,company,type,fields,category:'BL_COMPARISON',booking:id,version:1,history:[{title:'Independent extraction checked',detail:'Preset source values and field rules loaded. This is a demonstration record.',at:'20 Sep, 10:16'},{title:'SI and BL candidates identified',detail:'Document title and booking reference checked in the preset scenario.',at:'20 Sep, 10:15'},{title:'Email classified',detail:'BL keyword found; full message requests a comparison.',at:'20 Sep, 10:14'}],...extra};if(record.docIssue||record.pairIssue||record.processingError||record.processing){for(const f of Object.values(fields)){f.comparison=null;f.kind=record.docIssue?'MISSING':record.pairIssue?'AMBIGUOUS':null;f.reason='comparison_paused'}}return record}
