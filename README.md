...
- Numbers use string/BigInt decimal arithmetic, exact comparison and explicit `unset` or `en_comma` source profiles with evidence. No inferred locale or tolerance. A plain integer and canonical comma-grouped integer agree only with the same explicit source unit. Metric units and finite English number words are supported. Zero weight is allowed; negative weight and nonpositive/noninteger container counts block comparison. `6 × 40 HC` is six containers.
...
- Optional `TYPESAFE_API_KEY`: primary TypeSafe account key for Jev email routing, pinned to `jev-1.13.0` (override with `JEV_MODEL` after testing). `TYPESAFE_BACKUP_API_KEY` may hold a key from a separate account. The backup is called only when the primary request fails, is rate-limited, or returns an invalid contract; ordinary low-confidence decisions go directly to the independent Grafilab fallback instead of repeating the same judgment on another account. If both TypeSafe accounts are unavailable, Grafilab can still classify the original email. If that fallback also fails, the case remains a visible processing error. Explicit invoice queries retain the existing zero-call rule. Jev confidence below 0.8, conflicting comparison intent or incomplete message coverage falls back to the existing Grafilab classification path when configured. The second classifier sees the original email independently, never Jev's proposed answer. If the fallback is also uncertain, manual review remains required. Jev confidence is a routing signal, not guaranteed correctness. Its displayed excerpt is email context, not an explanation generated by Jev. Without a TypeSafe key, the previous routing remains available.
